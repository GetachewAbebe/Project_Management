...
class EvaluationAssignmentController extends Controller
{
    public function index()
    {
        ...
        $assignments = EvaluationAssignment::with(['project', 'evaluator'])
            ->latest()
            ->paginate(25)
            ->withQueryString();
    ...
    public function store(Request $request)
    {
        ...
        $hasAlreadyReviewed = Evaluation::where('project_id', $project->id)
            ->where('evaluator_id', $validated['evaluator_id'])
            ->exists();
        ...
        try {
            Mail::to($assignment->evaluator->email)
            ...
            Log::error('SMTP Dispatch Failure: '.$e->getMessage());
