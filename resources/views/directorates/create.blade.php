    ...
    <form action="{{ route('directorates.store') }}" method="POST">
        ...
        <div style="margin-bottom: 1.5rem;">
            ...
            <select name="research_center" style="width: 100%; padding: 0.8rem; border: 1px solid var(--border-color); border-radius: 8px; font-size: 1rem; font-family: 'Outfit', sans-serif;">
                ...
                <option value="biotech" {{ old('research_center') === 'biotech' ? 'selected' : '' }}>Biotech Research Center</option>
                <option value="emtech"  {{ old('research_center') === 'emtech'  ? 'selected' : '' }}>EmTech Research Center</option>
            </select>
